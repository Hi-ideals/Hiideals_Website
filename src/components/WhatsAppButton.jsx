...
export default function WhatsAppButton({ phone = '919999999999' }) {
    ...
    const onScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          // Hide on fast scroll up (>50px delta), show on scroll down or stop
          if (diff < -50) {
            setVisible(false)
  ...
  const message = encodeURIComponent('Hi Hiideals! I visited your website and would like to know more.')
    ...
    <AnimatePresence>
      {visible && (
        ...
        >
          ...
          <span className="absolute -inset-1 rounded-full animate-pulse opacity-10" style={{ background: '#25D366' }} />
          ...
          <motion.a
            ...
            className="relative w-14 h-14 rounded-full flex items-center justify-center shadow-lg shadow-green-500/30 hover:shadow-green-500/50 transition-shadow"
